...
import { BsFillCaretLeftFill } from 'react-icons/bs'; // Filled chevron arrow

const TestimonialCarousel: React.FC = () => {
  const [currentTestimonialIndex, setCurrentTestimonialIndex] = useState(0);
  const currentTestimonial: Testimonial = testimonialsData[currentTestimonialIndex];

  const handleAvatarClick = (index: number) => {
    setCurrentTestimonialIndex(index);
  };

  // Move selected testimonial to the top
  ...
  return (
    ...
    <h2
      className="text-[16px] uppercase tracking-widest text-gray-500 mb-12"
    ...
    <div className="flex flex-row justify-between items-start w-full gap-[100px] flex-nowrap">
      ...
      <div className="w-[70%]">
        <div className="text-[18px] md:text-4xl font-light leading-snug mb-8 relative">
          <span
            className="absolute text-[40px] md:text-[90px] -top-6 left-0 text-white"
      ...
      <div className="flex flex-col items-start space-y-8 w-[30%] md:ml-[100px]">
        {reorderedTestimonials.map((testimonial, index) => {
          ...
          const originalIndex = testimonialsData.findIndex(
